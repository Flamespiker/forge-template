...
**Current version: v12** (2026-09-01) — see the v12 update note below for what changed.

**v12 update (2026-09-01):** Phase 8 (Handoff Readiness) opened: **8.1, 8.2, and 8.3 checked off** — see each item's note below for the full detail. Landed alongside a docs/ reorg Mike did locally (ADRs → `docs/adr/`... resolved as staging-only, `core/decisions/` remains the one real ADR home per Document 7). Five commits, `main` unprotected on `forge-template` so no PR needed for these doc-only changes: `566ab53` (reorg), `61f52c0` (ADRs), `c98fe38` (Doc 6 v7), `b27eb1f` (Doc 7 v4), `3c9f58d` (this checklist). **8.4 and 8.5 both checked off in follow-up sessions (2026-09-01) — see each item's own note below. Phase 8 (Handoff Readiness) is now fully complete.** `01_FORGE_ProductSpec_v2.md` flagged as possibly needing its own review pass — out of Phase 8's defined scope (8.1/8.2 name only Documents 6 and 7), not pulled in.
...
- [x] 5.11 Document all fixes made during App 1 run — anything patched mid-run becomes a follow-up task *(see `docs/Archives/FORGE-Phase5-Closeout.md` §4–5 for the full list of confirmed structural gaps and manual interventions, and §7–8 for what's carried forward into Phase 6)*
...
- [x] 8.1 Final review of Document 6 (Orchestration Manager Guide) against everything learned in Phases 5–7 — update where reality differed from the doc *(2026-09-01 — v6→v7. Corrected two stale "Claude Agent SDK" references (ADR-0011 moved six of seven stages to the base `anthropic` client), corrected Gate 6 to reflect the confirmed-merge Deploy trigger requirement (Item #26), added Gate 2.5 for the Cost Estimator's `cost-approved` label (Item #34), added a new "Enhancement vs. Greenfield Requests" section (Items #24/#25/#28/#32), and added the post-deploy crash-loop health check to Failure Handling (Item #1). Commit `c98fe38`.)*
- [x] 8.2 Final review of Document 7 (Customization Reference) — confirm all locked/flexible/open items are accurate *(2026-09-01 — v3→v4. Same SDK correction; added Locked rows for the cost-approval gate, Enhancement-target resolution, the post-deploy health check, and the confirmed-merge Deploy requirement. Commit `b27eb1f`.)*
- [x] 8.3 Confirm all eleven ADRs in `core/decisions/` are fully written (not stubs) — the ten seed ADRs from Phase 1.8, plus ADR-0011 (organically added mid-build per step 3.1a, not part of the original seed set) *(2026-09-01 — nine of ten seed ADRs (0001–0007, 0009, 0010) were still genuine stubs; written from real source material in the architecture doc, Governance doc, and CLAUDE.md rather than invented. ADR-0008 and ADR-0011 were already real and left untouched. Commit `61f52c0`. Surfaced and resolved along the way: an ADR-storage-location question (Document 7 documents `core/decisions/` as the Locked path; a docs/ reorg had briefly staged copies in `docs/ADRs/` — resolved as staging-only, not a second permanent home, to avoid recreating the doc-duplication problem CLAUDE.md already warns about).)*
- [x] 8.4 Run the setup verification workflow (Phase 2.8–2.9) on a fresh clone — confirms a new Orchestration Manager can get from clone to verified config (including Managed Agents access) without hand-holding *(2026-09-01 — not a clean pass on the first try: the fresh-clone attempt surfaced a real bug first (Items #35/#36/#37 — `verify-setup.yml` and 8 other stage workflows hardcoded `forge-demo-apps`/`spike99`/`Flamespiker` instead of reading config; `team/config.yaml` itself had three mutually incompatible schemas across README/OM Guide/the real file). Fixed via new repo Variables (`FORGE_TARGET_REPO`, `FORGE_GITHUB_OWNER`, `FORGE_ADO_ORG_URL`) and a config-file schema trim — commits `53b3fd5`, `baddc8c`, `d40b761`, `5b8ace6`, `71424df`. Verified twice: a live run against the real setup (`https://github.com/Flamespiker/forge-template/actions/runs/33547029934`, both jobs green), then a genuine fresh-clone retest against a different target (`https://github.com/Flamespiker/forge-8-4b-scratch/actions/runs/33548880118` — repo since deleted per teardown below, but the run's own log confirmed the connectivity check resolved `Flamespiker/forge-8-4b-scratch` specifically, proving the fix reads `${{ vars.* }}` for real rather than coincidentally matching the old hardcoded values). Scratch GitHub App, repo, ADO PAT/work item, and resource group were all independently re-verified torn down afterward (fresh 404s/`az group exists`→`false`), nothing left standing. Full detail: `docs/FORGE-Open-Items-Backlog-v9.md`.)*
- [x] 8.5 Tag the repo `v1.0.0` — first stable release of the FORGE template *(2026-09-01 — annotated tag `v1.0.0` created on commit `97aa752a8ff0d89306dcbfb01c6a36b916066576` (`main` HEAD at the time, confirmed via `GET /repos/Flamespiker/forge-template/commits/main` and independently confirmed to include all three reconciliation commits from the same day — `64ded5c`, `adde747`, `97aa752` itself — as ancestors via `compare/{sha}...main`, `behind_by: 0` each). No prior `v1.0.0` tag existed (`GET .../tags` returned empty before creation). Pushed to origin; verified via `GET /repos/Flamespiker/forge-template/git/ref/tags/v1.0.0` (`object.type: "tag"`, confirming a genuine annotated tag object, not lightweight) and `GET .../git/tags/{tag_sha}` (message and `object.sha` both correct). Phase 8 (Handoff Readiness) is now fully complete, all five steps checked.)*
