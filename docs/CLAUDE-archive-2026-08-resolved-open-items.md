# CLAUDE.md — Archive: Resolved Open Items (through 2026-08-31)

Verbatim relocation of every resolved item's full narrative from CLAUDE.md's
"Open Items / Known Gaps" section, moved here 2026-08-31 to keep CLAUDE.md
(loaded into every session) lean. Item numbers are unchanged from CLAUDE.md so
cross-references elsewhere (the Open Items Backlog, context docs, other specs)
still resolve correctly — this is a relocation, not a renumbering.

Genuinely still-open items (#7, #11) remain in CLAUDE.md itself, in place — see
CLAUDE.md's "Open Items / Known Gaps" section. Every item below carries a short
pointer back in CLAUDE.md at the same item number.

---

## Item #1 — Deploy Agent had no app-secrets wiring mechanism, and no way to discover in advance that a given app needs a given secret
...
## Item #2 — REQ-2026-03's backend unit name doesn't fit Azure's Container App name length limit
...
## Item #3 — No pipeline stage validates that the app actually builds before Stage 6
...
## Item #4 — qa_agent.py's Jest/Vitest JSON parsing has a file-collection blind spot
...
## Item #5 — QA's _MAX_RETRIES = 3 only picks which label to apply — it never blocks or gates a re-run
...
## Item #6 — wait_for_all_threads_idle() can't distinguish "genuinely finished" from "every thread hit a fatal session-level error"
...
## Item #8 — CI-workflow scope creep
...
## Item #9 — Admin-merge pattern for ad hoc fix/* branches — 4 occurrences
...
## Item #10 — enforce_admins on forge-demo-apps' main branch protection was false
...
## Item #13 — A forge-template-level Dependency-Check suppression file
...
## Item #14 — Backend AzureAd config still placeholder — blocked real Azure AD login end-to-end
...
## Item #15 — Ad hoc PRs need the tracking-issue body line added manually
...
## Item #16 — Cleanup debt from the 2026-08-19 write-path verification session
...
## Item #17 — workflow_glue.py's resolve_feature_pr() couldn't find ad hoc fix PRs for Deploy
...
## Item #18 — New, previously-hidden bug uncovered by Item #17's live verification
...
## Item #19 — Dependabot alert triage pass completed 2026-08-21
...
## Item #20 — One real, one false-alarm next build failure
...
## Item #21 — Deploy Agent's _SHELL_TIMEOUT_SECONDS (1800s) was too tight for real frontend builds
...
## Item #22 — Deploy Agent didn't wire any scale rule for non-ingress worker units
...
## Item #23 — No on-demand way to verify a service's language build or Docker build outside the full pipeline
...
## Cross-cutting note — 2026-08-26 verification posture (Items #6, #8, #21, #22, #23)

Sat between Items #23 and #24 in CLAUDE.md's original numbered list — not itself a numbered item, but discusses only items that are now archived, so moved here rather than left orphaned in CLAUDE.md.
...
## Item #24 — Stage 3 (Implementation) never extended for Enhancement requests
...
## Item #25 — QA and Security both assumed services/<request_id>/, never services/<existing_service>/
...
## Item #26 — No human gate exists between a feature PR opening and Deploy firing
...
## Item #27 — 04-qa.yml's stale-label-clearing step decided by re-querying current label state instead of this run's outcome
...
## Item #28 — Deploy Agent (Stage 6) had zero Enhancement-target awareness
...
## Item #29 — README.md describes a materially different (partly aspirational, never-built) pipeline
...
## Item #30 — No security-check mechanism existed for a non-feature/*/non-design/* branch PR
...
## Item #31 — design_agent.py's (later, all 5 stage agents') _parse_model_json() had zero resilience to a malformed large JSON-mode response
