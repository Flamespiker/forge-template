...
**Supersedes:** v3 (2026-08-31, same day) — Mike decided the self-approval
deadlock this session; see below. No other content changed from v3.
...
surfaced from that discovery: **Item #32**, a real ADO-linkage design gap. The
PR self-approval/branch-protection deadlock — flagged as unresolved earlier this
same session — has now been **decided** (see below): Mike's call is to keep the
manual disable/restore workaround rather than build anything. What's left open:
Item #32 (new) — a design/process question for Mike, not a spec — two
...
**One item open requiring a decision:** Item #32 (below). The PR self-approval/
branch-protection deadlock that also lived here earlier this session has been
decided — see "Accepted ongoing process" below.
...
1. **Item #32** — a design/policy decision, no urgency, nothing else blocked on
...
5. **PR self-approval/branch-protection deadlock** — decided, not a to-do; the
