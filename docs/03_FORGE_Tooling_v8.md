...
**v8 changelog (2026-08-19):** §3.5 dependency vulnerability scanner default swapped from OWASP Dependency-Check to GitHub Dependabot alerts (native, API-based). Root cause: Dependency-Check timed out twice consecutively in CI at its 1800s ceiling (NVD sync far slower over GitHub Actions' network path than locally). See `docs/FORGE-DependencyScanner-Dependabot-Swap-Spec.md` for the full spec. Cost Summary (§8) and Provisioning Checklist (§4, item 8) updated to match.
...
| Tool | Role in FORGE | Required | Who Provisions | License Type | Build-Phase Cost | Production Cost |
|---|---|---|---|---|---|---|
| **Anthropic API** | Powers all ten agents. Nine agents (Codebase Ingestion, Intake, Requirements, Design, QA, Security, Deploy — plus the Stage 3 Implementation Coordinator and its Backend/Frontend/Test Writer subagents at the account level) run through the same Anthropic API account; billing splits by invocation type — see the Anthropic Python client and Managed Agents rows below | Required | Mike (build phase: personal developer account); org IT or a designated API owner (production) | Consumption-based (pay-per-token, plus Managed Agents session-hour billing for Stage 3 — see below) | Mike's personal API account — token costs incurred per pipeline run | Org API account; monthly cost depends on run volume and average token consumption per stage |
| **Anthropic Python client (`anthropic` package)** | The Python package FORGE uses to call the Anthropic Messages API directly for all stages except Stage 3 — installed in GitHub Actions runner environments via pip (ADR-0011; supersedes the Claude Agent SDK approach specified in earlier versions of this document) | Required | No separate account; bundled with Anthropic API access | Open source (MIT) | Free | Free |
...
**ADR-0011 note (decided chat 21, verified chat 22):** Earlier versions of this document (through v5) specified the Claude Agent SDK (`claude-agent-sdk`) as the invocation mechanism for all stages except Stage 3. Phase 3.1 live testing found every Agent SDK call — even a trivial, tool-free text exchange — paid a fixed cost: ~25,700 tokens of Claude Code's bundled system prompt and tool definitions written to cache on a cold call (~$0.10 at $3.75/MTok cache-write rates), plus a subprocess-launch latency floor measured at ~10 seconds, regardless of task size. None of the six non-Stage-3 stages use the SDK's autonomous tool-execution capability — FORGE's deterministic Python layer performs all file I/O for these stages; every Phase 3.1 invocation across them already passed `allowed_tools=[]`. FORGE now calls the base `anthropic` Python client directly against the Messages API for these six stages, eliminating that overhead with no loss of capability. Stage 3 (Managed Agents, ADR-0010) is unaffected — it was never built on the Agent SDK. Full text in `core/decisions/0011-base-anthropic-client.md`.

**Cost estimation note:** Exact token costs per pipeline run depend on codebase size, requirements complexity, and agent prompt lengths — these can only be measured during the build-phase runs. After App 1 and App 2 runs are complete, a per-pipeline cost estimate should be recorded here. As a rough planning figure: Sonnet-tier models at typical requirements-to-deployment run volumes (10–50k tokens per stage, 10 stages) are likely to run in the **$0.50–3 USD** range per complete pipeline execution for token costs alone, plus an estimated $0.08–0.32 USD per run for Managed Agents session-hour billing on Stage 3 (1–4 hours active runtime), though both figures will vary significantly. Track actuals. *(Revised down from the earlier $1–5 USD estimate to reflect ADR-0011: the six non-Stage-3 stages no longer pay the Agent SDK's ~$0.10 cold-call overhead per invocation, which the original figure was set before Phase 3.1 testing had surfaced.)*
...
| **GitHub Dependabot alerts** | Dependency vulnerability scanning | Native GitHub feature — alerts on known-vulnerable NuGet/npm dependencies via the repo's dependency graph, pre-computed severity (critical/high/medium/low). API-based (`GET .../dependabot/alerts`), not an Actions job — no per-run scan step, no NVD sync, no CI timeout exposure | Required (one dependency scanner must be chosen) | Org admin enables Dependabot alerts + dependency graph on the repo (Settings → Code security and analysis); Orchestration Manager grants the FORGE GitHub App's installation the "Dependabot alerts: Read-only" permission | Free (native GitHub.com feature, not part of the paid Advanced Security add-on) | Free | Free |
...
| **OWASP Dependency-Check** | Dependency vulnerability scanning (alternative) | Original FORGE default through v7 of this document; scans .NET NuGet and Node npm dependencies against the NVD CVE database directly. Superseded by Dependabot alerts (above) after repeated CI timeouts — retained here as a documented alternative for orgs without Dependabot access (e.g., GitHub Enterprise Server configurations where it isn't available) | Optional (legacy/alternative) | Orchestration Manager adds `dependency-check/Dependency-Check_Action` to the Security workflow | Open source (Apache 2.0) | Free | Free |
| **Snyk** | Dependency vulnerability scanning (alternative) | Freemium SaaS alternative; richer advisory database, auto-fix PR suggestions | Optional | Orchestration Manager creates Snyk account; integrates via `snyk/actions` | Freemium (free tier: limited scans/month; paid plans from ~$25/month) | Free tier sufficient for build phase | Evaluate based on scan volume |

**Default recommendation:** Semgrep Community + Gitleaks + GitHub Dependabot alerts. All three cost nothing and satisfy the Security Agent's three check categories without a SaaS dependency. Dependabot alerts replaced OWASP Dependency-Check as the default (2026-08-19) after Dependency-Check timed out twice consecutively in CI at its 1800s ceiling — see the Security Tooling note below. GitHub Advanced Security and Snyk remain the upgrade path if LAA decides to consolidate security tooling or wants deeper SAST coverage.

**Dependabot swap note (2026-08-19):** confirmed live — the Security Agent step in CI dropped from two prior 30-minute timeouts to 18 seconds after the swap. Two one-time prerequisites, confirmed present via live API checks rather than assumed from "done": Dependabot alerts + dependency graph enabled on the repo, and the `forge-pipeline` GitHub App's installation granted the "Dependabot alerts: Read-only" permission (re-approval required after granting). Native per-alert dismissal (`PATCH /repos/{owner}/{repo}/dependabot/alerts/{n}`) replaces the old suppression-file mechanism for dev-only findings; dismissal stays a manual human action, not something the Security Agent does automatically. Full detail: `docs/FORGE-DependencyScanner-Dependabot-Swap-Spec.md`.
...
8. **Configure security tools** — Orchestration Manager adds Semgrep and Gitleaks GitHub Actions steps to the Security workflow; enables Dependabot alerts + dependency graph on the repo (Settings → Code security and analysis) and grants the `forge-pipeline` GitHub App's installation the "Dependabot alerts: Read-only" permission (re-approve the installation after granting). No external accounts required for the defaults.
...
| Anthropic API (token costs) | Personal API account; ~$0.50–3 USD per full pipeline run (estimate — track actuals; revised down from $1–5 per ADR-0011's SDK-overhead removal) | Org API account; same per-run cost, multiplied by run volume |
...
| Security tools (Semgrep, Gitleaks, GitHub Dependabot alerts) | Free | Free |
...
- **Claude Sonnet 5 introductory pricing** — Sonnet 5 is currently available at introductory pricing ($2/$10 per MTok input/output) through 2026-08-31, cheaper than the Sonnet 4.6 tier FORGE uses as its default (§3.3, Claude model tiers row). Flag as a future cost-optimization candidate once the introductory window closes and post-window pricing is confirmed — not an action item for the current build phase.
