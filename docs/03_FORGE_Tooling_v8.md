...
A Critical finding from any of the three tools sets a failing check run on the implementation PR (Document 2 §4.7), which blocks merge without manual override — this is enforced by the branch protection rule requiring the `security-check` check to pass, not by the agent making a judgment call. **Scanner-failure gap closed 2026-08-13:** a scanner that fails to execute at all (crash, timeout, missing report — distinct from running clean with zero findings) is now treated the same as a Critical finding for gating purposes — `security-approved` is withheld and the check run fails with a title distinguishing "incomplete — scanner failure" from a real block, so a red X is never misread as "vulnerabilities found" when the true state is "we don't know yet." Fixed and verified per `FORGE-SecurityAgent-ScannerFailureVerdict-Spec.md` (`core/agents/security_agent.py`, commit `5492305`).
