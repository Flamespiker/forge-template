...
**Last Updated:** 2026-07-30 (Phase 3 continues — steps 3.4a–3.7 now genuinely complete: real run committed, PR #5 merged on `forge-demo-apps`, all four review items closed, and a new living cost log (`docs/FORGE-pipeline-cost-log.md`) created to track per-run actuals going forward — chat 29)  
...
- **QA Agent** — runs tests, files bugs, loop-back to implementation (Messages API call)
- **Security Agent** — SAST, OWASP, secrets detection, parallel with QA (Messages API call)
- **Deploy Agent** — containerizes, pushes to Azure Container Apps (Messages API call)
