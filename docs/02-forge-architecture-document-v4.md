...
- **Each stage's agent invocation is stateless.** Every stage spins up a fresh, bounded agent invocation scoped to that stage, given exactly the artifacts it needs as input, and producing exactly the artifacts the next stage (or the human gate) needs as output. No agent process persists across stages or holds cross-stage memory. This keeps every run reproducible, keeps agent scope bounded (per Document 0's core pattern), and matches how GitHub Actions runs work anyway — each job starts from a clean runner.

  *Invocation mechanism (per ADR-0011):* Six of the seven stages (Intake, Requirements, Design, QA, Security, Deploy) call Claude directly via the base `anthropic` Python client's Messages API — a single-turn request/response with a per-stage system prompt and no autonomous tool-use loop, since FORGE's own Python layer handles all file I/O for these stages. Stage 3 (Implementation) is the exception, using Anthropic Managed Agents (ADR-0010) — a separate mechanism entirely, not the Messages API called directly. Neither mechanism is the Claude Agent SDK, which was the original (now superseded) design for the six non-Stage-3 stages.
...
- A place to run bounded, ephemeral compute (the per-stage Anthropic Messages API calls, and the Stage 3 Managed Agents session)
...
3. Invokes the relevant agent call as a bounded job — a single-turn `anthropic` Messages API call for six of the seven stages (ADR-0011), or the Stage 3 Managed Agents coordinator session (see exception below).
...
*Exception — Stage 3 (Implementation):* Step 3 invokes a Managed Agents coordinator agent session rather than a direct `anthropic` Messages API call. The coordinator manages Backend, Frontend, and Test Writer subagents in parallel via the Anthropic Managed Agents API (`managed-agents-2026-04-01` beta header). The GitHub Actions workflow starts the agent session and waits for its completion; all parallel subagent orchestration happens within the Managed Agents runtime, not within GitHub Actions.
...
Six of the seven stages — every stage except Implementation — invoke Claude via the base **`anthropic`** Python client, calling the Messages API directly as a scoped, single-purpose call within a GitHub Actions job: a single-turn request/response with a per-stage system prompt, no autonomous tool-use loop (ADR-0011). Stage 3 (Implementation) is the exception, using an Anthropic Managed Agents coordinator session (ADR-0010) — described in its own rows below. None of them run continuously; each is instantiated, given its stage's inputs, produces its stage's outputs, and exits.
...
Intake spreadsheet row                                            [FORGE repo]
   → GitHub tracking issue (created at intake)                    [FORGE repo]
      → requirements.md + ado-work-items.json committed to
        the pipeline-state branch (not main)                      [monorepo, pipeline-state branch]
         → ADO User Story (created on requirements-approved,
           linked to tracking issue)                              [ADO, cross-linked to FORGE repo]
            → design/<request-id> branch + PR (Design Agent
              reads requirements.md from pipeline-state)          [monorepo, cross-linked to FORGE tracking issue]
...
**On the `pipeline-state` branch specifically:** `main` in the monorepo is branch-protected (§6, required reviewers) to gate merges of real application code — but Requirements and ADO-item-creation write orchestration state (`requirements.md`, `ado-work-items.json`), not application code, and do so directly rather than through a PR (§6 branching strategy covers `feature/<request-id>` and `design/<request-id>` only; requirements-stage writes were never meant to need human review the way a code change does). Protecting `main` therefore meant giving these direct commits a home that isn't `main` — `pipeline-state` is that home: a persistent, intentionally-unprotected branch holding orchestration state, read directly by every downstream stage that needs it (`create_ado_items.py`, `design_agent.py`, `qa_agent.py`). This doesn't change ADR-0002's "state lives in committed files, not agent memory" principle — it's still committed files, just on a branch scoped for that purpose rather than `main`.
...
Summary: Stage 3 uses an Anthropic Managed Agents coordinator agent session with three specialist subagents (Backend, Frontend, Test Writer) running in parallel on a shared sandbox filesystem, rather than three independent GitHub Actions parallel jobs. All other stages use stateless, single-turn calls to the base `anthropic` Python client's Messages API (ADR-0011). ADR-0002 (stateless per stage) remains valid — "stateless" means no cross-stage memory; within-stage coordination is permitted and bounded by the stage window.
...
## 12. ADR-0011 — Base Anthropic Client for Non-Stage-3 Agent Invocation

Full text in `core/decisions/` (see the seed ADR list, Document 4) — filename not yet confirmed against the actual file in this document; verify against the repo before citing a path elsewhere.

Summary: The six non-Stage-3 stages (Intake, Requirements, Design, QA, Security, Deploy) switched from the Claude Agent SDK to the base `anthropic` Python client, calling the Messages API directly with a per-stage system prompt. This is a single-turn, tool-free call — none of these six stages ever exercised the Agent SDK's autonomous tool-use loop; FORGE's own Python layer already performed all file I/O for them. Stage 3 (Implementation) is unaffected — it uses Anthropic Managed Agents (ADR-0010), a separate mechanism from the Agent SDK entirely. The `invoke_agent()` function signature and `AgentResult` return contract in `core/agents/utils/claude_agent_wrapper.py` are preserved as closely as possible; `allowed_tools` is removed from the interface, since there is no tool-use loop left to scope.

Consequences to note during build phase:
- Eliminates the ~$0.10 cold-call cost floor and ~25,700-token cache-creation tax on cold calls for five-sixths of the pipeline's agent invocations
- Eliminates the ~10-second subprocess-launch latency floor per invocation, and removes the bundled Claude Code CLI dependency for these six stages
- Drops a dependency on `claude-agent-sdk` (Alpha-classified, near-daily release cadence, at least one prior breaking rename) for the five-sixths of the pipeline that never needed its agentic capabilities
- Loses the Agent SDK's automatic per-invocation cost computation — FORGE now maintains its own per-model token-rate table in the wrapper, which needs manual updates if Anthropic's published pricing changes
- Loses latent tool-use capability for these six stages; a future stage genuinely needing autonomous tool-calling would use the Messages API's own `tools` parameter (scoped, per-call function definitions) or a documented, stage-specific reintroduction of the Agent SDK — not a silent revert

---

## 13. Where This Fits

This document defines *how*. The next document — the **Tool & Licensing Inventory** — enumerates every tool named here (GitHub, ADO, Azure Container Apps, the `anthropic` Python client, Anthropic Managed Agents, etc.), who provisions each, what's required vs. optional, and cost, so the architecture above is fully provisionable rather than just designed.
