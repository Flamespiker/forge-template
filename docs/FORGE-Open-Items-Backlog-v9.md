# FORGE — Open Items Backlog: Planning for Next Session(s) (v9)

**Prepared:** 2026-09-01 (Claude.ai)
**Supersedes:** v8 (2026-09-01, same day) — the fresh-clone scratch retest
ran and passed. **8.4 is genuinely closed.** 8.5 (`v1.0.0` tag) is now
unblocked.
...
1. **8.5 — tag `v1.0.0`** (immediate next step, now unblocked). 8.4's
   fresh-clone retest passed clean:
   `https://github.com/Flamespiker/forge-8-4b-scratch/actions/runs/33548880118`
   — both jobs green, connectivity check resolved the scratch target
   (`forge-8-4b-scratch`) specifically, not `forge-demo-apps`, proving
   the `${{ vars.* }}` fix is real indirection, not coincidence. Full
   teardown independently re-verified, nothing left standing, near-$0
   cost. One minor note from teardown: Claude Code CLI's `gh` auth
   still lacks `delete_repo` scope (had to have Mike delete the scratch
   repo manually) — not blocking, but worth fixing the token scope
   before the next scratch-repo exercise so teardown doesn't need a
   manual step.
